...
namespace Manager.TexasPoker.ApiControllers
{
    ...
    public class BuyVIPEventsController : ApiControllerBase
    {
        ...
        public async Task<PaginationResponse<BuyVIPEventEntity>> Get(
            ...
            int count = 1000)
        {
            ...
            var total = await this.Database.BuyVIPEvents.CountAsync(filter, token);
            ...
            var model = new PaginationResponse<BuyVIPEventEntity>
            {
                Page = page,
                PageSize = count,
                Raws = result,
        ...
        public async Task<BuyVIPEventEntity> Get(string id, CancellationToken token)
        {
            var result = await this.Database.BuyVIPEvents.GetSingleAsync(id, token);
