...
namespace TexasPoker.Manager.Api.ApiControllers
{
    ...
    public class PlayerReportEventsController : ApiControllerBase
    {
        ...
        public async Task<PluginPaginationResponse<PlayerReportEventEntity>> Post([FromBody]PluginRequestInfo request)
        {
            ...
            var total = await this.Database.PlayerReportEvents.CountAsync(filter, CancellationToken.None);
            ...
            var model = new PluginPaginationResponse<PlayerReportEventEntity>
            {
                Page = page,
                PageSize = count,
                Raws = result,
