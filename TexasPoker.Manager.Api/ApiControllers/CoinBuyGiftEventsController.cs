...
namespace TexasPoker.Manager.Api.ApiControllers
{
    ...
    public class CoinBuyGiftEventsController : ApiControllerBase
    {
        ...
        public async Task<PluginPaginationResponse<CoinBuyGiftEventEntity>> Post([FromBody]PluginRequestInfo request)
        {
            ...
            var total = await this.Database.CoinBuyGiftEvents.CountAsync(filter, CancellationToken.None);
            ...
            var model = new PluginPaginationResponse<CoinBuyGiftEventEntity>
            {
                Page = page,
                PageSize = count,
                Raws = result,
