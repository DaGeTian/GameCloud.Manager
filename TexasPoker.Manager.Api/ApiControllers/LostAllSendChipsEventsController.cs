...
namespace TexasPoker.Manager.Api.ApiControllers
{
    ...
    public class LostAllSendChipsEventsController : ApiControllerBase
    {
        ...
        public async Task<PluginPaginationResponse<LostAllSendChipsEventEntity>> Post([FromBody]PluginRequestInfo request)
        {
            ...
            var total = await this.Database.LostAllSendChipsEvents.CountAsync(filter, CancellationToken.None);
            ...
            var model = new PluginPaginationResponse<LostAllSendChipsEventEntity>
            {
                Page = page,
                PageSize = count,
                Raws = result,
