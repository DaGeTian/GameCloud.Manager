...
namespace Manager.TexasPoker.ApiControllers
{
    ...
    public class TexasPokerEventsController : ApiControllerBase
    {
            ...
            int count = 1000)
        {
            ...
            var model = new PaginationResponse<TexasPokerEventEntity>
            {
                Page = page,
                PageSize = count,
                Raws = result,
        ...
        public async Task<TexasPokerEventEntity> Get(string id, CancellationToken token)
        {
            var result = await this.Database.TexasPokerEvents.GetSingleAsync(id, token);
