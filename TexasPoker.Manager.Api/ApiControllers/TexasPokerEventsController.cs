...
namespace TexasPoker.Manager.Api.ApiControllers
{
    ...
    public class TexasPokerEventsController : ApiControllerBase
    {
        /// <summary>
        ...
        public TexasPokerEventsController(TexasPokerDatabaseContext database, Settings settings)
        ...
        [Route("api/events/TexasPoker")]
        public async Task<PluginPaginationResponse<TexasPokerEventEntity>> Post([FromBody]PluginRequestInfo request)
        {
            ...
            Expression<Func<TexasPokerEventEntity, bool>> filter = null;
            ...
            var total = await this.Database.TexasPokerEvents.CountAsync(filter, CancellationToken.None);

            IQueryable<TexasPokerEventEntity> queryable = this.Database.TexasPokerEvents.Collection.AsQueryable();
            if (filter != null)
            {
            ...
            var model = new PluginPaginationResponse<TexasPokerEventEntity>
            {
                Page = page,
                PageSize = count,
                Raws = result,
