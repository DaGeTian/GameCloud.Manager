...
namespace TexasPoker.Manager.Api.ApiControllers
{
    ...
    public class CoinBuyItemEventsController : ApiControllerBase
    {
        ...
        public async Task<PluginPaginationResponse<CoinBuyItemEventEntity>> Post([FromBody]PluginRequestInfo request)
        {
            ...
            var total = await this.Database.CoinBuyItemEvents.CountAsync(filter, CancellationToken.None);
            ...
            var model = new PluginPaginationResponse<CoinBuyItemEventEntity>
            {
                Page = page,
                PageSize = count,
                Raws = result,
