...
namespace TexasPoker.Manager.Api.ApiControllers
{
    ...
    public class BuyCoinsEventsController : ApiControllerBase
    {
        ...
        public async Task<PluginPaginationResponse<BuyCoinsEventEntity>> Post([FromBody]PluginRequestInfo request)
        {
            ...
            var total = await this.Database.BuyCoinsEvents.CountAsync(filter, CancellationToken.None);
            ...
            var model = new PluginPaginationResponse<BuyCoinsEventEntity>
            {
                Page = page,
                PageSize = count,
                Raws = result,
